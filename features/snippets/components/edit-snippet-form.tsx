...
export function EditSnippetForm({
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const parsedValues = updateSnippetSchema.safeParse({
      code,
      language,
      isAnonymous,
    });
    ...
    try {
      ...
      setFormError("Une erreur est survenue lors de la modification.");
    } finally {
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={onSubmit}>
      ...
      <button
        aria-label="Modifier le snippet"
        className="btn-primary h-11"
        type="submit"
        disabled={isSubmitting}
      >
        {isSubmitting ? "Modification..." : "Modifier le snippet"}
      </button>
    </form>
  );
}
